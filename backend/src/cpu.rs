...
macro_rules! abs {
    ($f:ident) => {
        &[
            ...
            fetch_high_and_inc_pc,
            $f,
            fetch_opc_and_inc_pc,
        ]
    };
}
...
macro_rules! zpg {
    ($f:ident) => {
        &[fetch_low_and_inc_pc, $f, fetch_opc_and_inc_pc]
    };
}
...
static OPC_LUT: [&[fn(&mut Emu)]; 0x100] = [
    ...
    &[],                              // 0xA7
    &[],                              // 0xA8
    &[imm_lda, fetch_opc_and_inc_pc], // 0xA9
    &[],                              // 0xAA
    &[],                              // 0xAB
    abs!(ldy),                        // 0xAC
    abs!(lda),                        // 0xAD
...
fn fetch_opc_and_inc_pc(emu: &mut Emu) {
